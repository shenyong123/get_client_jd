...
# connect to mysqldb
def getConnect(strHost,strUser,strPasswd,port=3307):
    conn = MySQLdb.connect(host=strHost,user=strUser,passwd=strPasswd,port=port,charset='utf8')
    return conn
...
class MySQLQuery(object):
    ...
    def sqlQuery(self,ku_id,corporation_id):
    ...
    def get_results(self,ku_id,corporation_id):
        if ku_id and corporation_id:
            curs = initClientEncode(self.conn)
            sql = self.sqlQuery(ku_id,corporation_id)
            print(sql)
            curs.execcute(sql)
...
#134
conn2 = getConnect('192.168.8.134','kdd','GeYFL4EU,tcOtP$W',port=3307)
mysql = MySQLQuery(conn2)
for i in range(0,16,2):
    infos = mysql.sqlQuery(str(i),1228443)
    for info in infos:
        print info
